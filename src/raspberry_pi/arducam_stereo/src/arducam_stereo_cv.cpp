...
rclcpp::Node::SharedPtr arducam_node;
...
int main(int argc, char* argv[]) {
    ...
    std::shared_ptr<ArducamStereo> arducam = std::make_shared<ArducamStereo>();
    arducam_node = rclcpp::Node::make_shared("arducam");
    image_transport::ImageTransport it(arducam_node);
    arducam->start_stream();
    rclcpp::shutdown();
    return 0;
}
